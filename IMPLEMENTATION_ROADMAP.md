...
- **Colors/spacing/type** → copy tokens from §4–§7
- **Cards** → §10 structure mandatory for listing surfaces
- **Booking** → §12 for detail page work
...
```
Phase 1  → Design foundation stabilization     [UNLOCKS: consistent AI output, all UI phases]
Phase 2  → Mobile navigation & core UX        [UNLOCKS: safe-area for sticky CTAs]
Phase 3  → Listing details optimization       [UNLOCKS: conversion baseline for launch]
Phase 4  → Create listing wizard              [UNLOCKS: supply growth]
Phase 5  → My Toys dashboard                  [UNLOCKS: owner retention]
Phase 6  → Admin moderation UX polish         [UNLOCKS: faster inventory approval]
Phase 7  → Empty states & trust polish        [UNLOCKS: launch-quality feel]
Phase 8  → Production stabilization           [UNLOCKS: deploy]
Phase 9  → Real user testing prep             [UNLOCKS: public launch]
```
